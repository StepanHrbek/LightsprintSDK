...
namespace rr /// LightsprintCore - graphics API independent realtime global illumination solver.
{
	...
	struct RR_API RRMatrix3x4Ex : public RRMatrix3x4
	{
		//! Inverse matrix.
		RRMatrix3x4 inverse;
		...
		RRMatrix3x4Ex() {};
		explicit RRMatrix3x4Ex(const RRMatrix3x4& a);
		...
		//
		//! Unlike transformDirection(), transformNormal() ensures that normals perpendicular to surface
		...
		//
		//! Unlike getTransformedDirection(), getTransformedNormal() ensures that normals perpendicular to surface
		//! stay perpendicular even for matrices with non-uniform scale.
