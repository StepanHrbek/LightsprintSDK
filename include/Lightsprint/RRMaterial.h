...
namespace rr
{
	...
	struct RRRadiometricMeasure
	{
		...
		bool exiting : 1; ///< Selects between [0] incoming radiation (does not include emittance) and [1] reflected+emited radiation. \n Typical setting: 0.
	...
	#define RM_RADIOSITY_LINEAR             rr::RRRadiometricMeasure(1,0,0,1,1)
	#define RM_RADIOSITY_CUSTOM             rr::RRRadiometricMeasure(1,1,0,1,1)
	...
	struct RR_API RRMaterial : public RRUniformlyAllocatedNonCopyable
	{
		...
		//!   Unwrap can be imported or automatically generated by RRObjects::buildUnwrap().
		//!
		//! lightmap.color
		//! - Not used. Keep it set to 0.5.
		//!
		//! lightmap.texture
		//! - Secondary location, it is usually nullptr, because we store lightmaps and ambient maps in separated layers in RRObjectIllumination.
		//!   However, some fileformats store them in ambient or lightmap slot in material.
		//!   We use lightmap.texture only for smoother import from / export to such fileformats.
		//!   After import, we move lightmaps from RRMaterial to lightmap layer in RRObjectIllumination.
		Property      lightmap;
