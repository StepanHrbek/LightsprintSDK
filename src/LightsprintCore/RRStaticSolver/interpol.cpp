...
bool Object::buildTopIVertices(const RRDynamicSolver::SmoothingParameters* smoothing, bool& aborting)
{
	...
	//  we don't want to stitch them on higher level, stitched multiobject would require !indexed render (complicated to ensure, slower)
...
// nepokracuj do autosmoothingu, je rozbity protoze v mergeCloseIVertices dal pocita s 'vertices' misto 'stitchedVertices'
//	if (smoothing->maxSmoothAngle<=0)
	{
		// delete stitched vertices
		if (stitchedMesh!=originalMesh) delete stitchedMesh;
	...
	unsigned numIVertices = stitchedVertices;
	//printf("IVertices loaded: %d\n",numIVertices);
#ifdef SUPPORT_MIN_FEATURE_SIZE
	// volano jen pokud ma neco delat -> malinka uspora casu
	if (smoothing->minFeatureSize>0 && !aborting)
	{
		...
		numIVertices -= mergeCloseIVertices(topivertexArray,smoothing->minFeatureSize,aborting);
		//printf("IVertices after merge close: %d\n",numIVertices);
	}
#endif

	// split ivertices with too different normals
	for (unsigned v=0;v<stitchedVertices;v++)
	{
		if (aborting) break;
		RRMesh::Vertex vert;
		stitchedMesh->getVertex(v,vert);
		numIVertices += topivertexArray[v].splitTopLevelByAngleNew((RRVec3*)&vert,this,smoothing->maxSmoothAngle,outOfMemory);
