...
unsigned RRSolver::updateLightmap(int objectNumber, RRBuffer* buffer, RRBuffer* directionalLightmaps[3], RRBuffer* bentNormals, const UpdateParameters* _params, const FilteringParameters* _filtering)
{
	...
	unsigned vertexBufferWidth = 0;
	{
		...
		else
		{
			...
			{
				RRReporter::report(WARN,"Invalid objectNumber (%d, valid is 0..%zd).\n",objectNumber,getStaticObjects().size()-1);
				return 0;
			}
...
unsigned RRSolver::updateLightmaps(int layerLightmap, int layerDirectionalLightmap, int layerBentNormals, const UpdateParameters* _params, const FilteringParameters* _filtering)
{
	...
	if (containsFirstGather)
	{
		...
		paramsDirect.measure_internal.direct = true; // [#41] probably has no effect
			...
			for (unsigned i=0;i<NUM_BUFFERS;i++)
			{
				if (allLayers[i]>=0)
				{
					...
					if (vertexBuffer)
					{
						...
						{
							RR_LIMITED_TIMES(1,RRReporter::report(WARN,"Directional buffers not updated in 'realtime' mode (quality=0, useCurrentSolution only).\n"));
						}
					}
				}
			}
