...
unsigned RRSolver::updateLightmaps(int layerLightmap, int layerDirectionalLightmap, int layerBentNormals, const UpdateParameters* _paramsDirect, const UpdateParameters* _paramsIndirect, const FilteringParameters* _filtering)
{
	...
	if (containsFirstGather)
	{
		...
		paramsDirect.currentSolutionMultiplier = 1; // set solution generated here to be gathered in final gather
			...
			for (unsigned i=0;i<NUM_BUFFERS;i++)
			{
				if (allLayers[i]>=0)
				{
					...
					if (vertexBuffer)
					{
						...
						{
							RR_LIMITED_TIMES(1,RRReporter::report(WARN,"Directional buffers not updated in 'realtime' mode (quality=0, currentSolutionMultiplier only).\n"));
						}
					}
				}
			}
