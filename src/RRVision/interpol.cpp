...
void IVertex::insert(Node *node,bool toplevel,real power,Point3 apoint)
{
	...
	if(RRScene::GetStateF(RRScene::RRSSF_SUBDIVISION_SPEED)==0)
		power *= node->area;
...
{
	bool getSource=RRScene::GetState(RRScene::RRSS_GET_SOURCE)!=0;
	bool getReflected=RRScene::GetState(RRScene::RRSS_GET_REFLECTED)!=0;
...
*/
{
	...
	while(1)
	{
			...
			if(ivertexInfo[ivertex1Idx].ourVertices.size())
		{
			...
			if(minDist<=RRScene::GetStateF(RRScene::RRSSF_MIN_FEATURE_SIZE)) break;
		}
...
void Object::buildTopIVertices()
{
	...
	if(RRScene::GetStateF(RRScene::RRSSF_MIN_FEATURE_SIZE)>0)
	{
...
{
	if(RRScene::GetState(RRScene::RRSS_FIGHT_NEEDLES)<2) return Channels(0); // pokud nebojujem s jehlama, vertexum bez powerTopLevel (zrejme nekde v jehlach na okraji sceny) dame barvu 0
