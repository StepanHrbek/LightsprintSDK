...
#define MESHING              RRScene::GetStateF(RRScene::RRSSF_SUBDIVISION_SPEED)    // 0.3=few meshes, 1=default, 3=many meshes (noise)
...
again:
	...
	if(sina<=0) return -6;
	if(area<=0) return -4;
	if(area<=RRScene::GetStateF(RRScene::RRSSF_IGNORE_SMALLER_AREA)) return -5;
	...
	if(minangle<=RRScene::GetStateF(RRScene::RRSSF_IGNORE_SMALLER_ANGLE)) return -14;
	isNeedle = RRScene::GetState(RRScene::RRSS_FIGHT_NEEDLES) && minangle<=RRScene::GetStateF(RRScene::RRSSF_FIGHT_SMALLER_ANGLE);
	...
	if(area<=RRScene::GetStateF(RRScene::RRSSF_IGNORE_SMALLER_AREA)) return -12;
	isNeedle |= RRScene::GetState(RRScene::RRSS_FIGHT_NEEDLES) && area<=RRScene::GetStateF(RRScene::RRSSF_FIGHT_SMALLER_AREA);
...
Channels Scene::gatherIrradiance(Point3 point,Vec3 normal,Triangle *skip,Channels power)
{
	...
	unsigned numRays = RRScene::GetState(RRScene::RRSS_GET_FINAL_GATHER);
