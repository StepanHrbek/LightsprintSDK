...
#define MESHING              1    // 0.3=few meshes, 3=many meshes (noise)
...
#define PHOTOMETRIC_R        0.265
#define PHOTOMETRIC_G        0.670
#define PHOTOMETRIC_B        0.065
...
Color __colorFilter={0.33,0.33,0.33}; // see core.h
...
void SubTriangle::splitGeometry(IVertex *asubvertex)
{
	...
	if(splitVertex_rightLeft>=0)
	{
		bool rl=splitVertex_rightLeft/3;
...
static real spicatost(real a,real b,real c) // delky stran
{
	...
	return (lo+mid-hi<=0)?1e10:(hi/(lo+mid-hi)); // spicatost, 1..1000 pohoda, 1000..nekonecno jehla
}
...
again:
	...
	#ifdef ALLOW_DEGENS
	if(psqr<=0) {psqr=0.0001;printf("Low numerical quality, fixing area=0 triangle.\n");} else
	if(psqr>=4) {psqr=3.9999;printf("Low numerical quality, fixing area=0 triangle.\n");}
	#endif
	real cosa=1-psqr/2;
...
char Scene::selectColorFilter(int i)
{
	...
	Color myColorFilter[4]={{1,0,0},{0,1,0},{0,0,1},{0.33,0.33,0.33}};
...
//        s->diffuseReflectance=s->_rd*(__colorFilter[0]*s->_rdcx+__colorFilter[1]*s->_rdcy+__colorFilter[2]*(1-s->_rdcx-s->_rdcy));
	  s->diffuseReflectance=s->_rd*(__colorFilter[0]*r+__colorFilter[1]*g+__colorFilter[2]*b)/(PHOTOMETRIC_R*r+PHOTOMETRIC_G*g+PHOTOMETRIC_B*b+0.01);
	}
...
real Triangle::setSurface(Surface *s)
{
	...
	real filteringCoef=(__colorFilter[0]*r+__colorFilter[1]*g+__colorFilter[2]*b)/(PHOTOMETRIC_R*r+PHOTOMETRIC_G*g+PHOTOMETRIC_B*b+0.01);
	...
	if(!bests && nodes)
	{
		...
		for(unsigned i=0;i<nodes;i++) if(node[i]->shooter)
		{
			...
			if(distributing) {
			  ...
			  real distribValue=toDiffuse/(node[i]->shooter->factors()+0.5)*DISTRIB_TIME;
			  real refreshValue=allEnergy/(node[i]->shooter->shotsForFactors+0.5)*REFRESH_TIME;
...
void Scene::resetStaticIllumination(bool preserveFactors)
{
...
public:
	...
	{
		unsigned n=num++;
		static const real dir[4][3]={{0,0,-0.5},{0,1,0.5},{0.86602540378444,-0.5,0.5},{-0.86602540378444,-0.5,0.5}};
		real x=0;
		real y=0;
		real dist=1;
		while(n)
...
void Scene::refreshFormFactorsFromUntil(Node *source,real accuracy,bool endfunc(Scene *))
{
	DBGLINE
	if(phase==0)
	{
	...
	if(phase==1)
	{
		...
		while(shotsAccumulated<shotsForNewFactors)
		{
			...
			shotsAccumulated++;
			shotsTotal++;
			if(shotsTotal%10==0) if(endfunc(this)) return;
		}
		phase=2;
	}
	if(phase==2)
...
bool Scene::energyFromDistributedUntil(Node *source,bool endfunc(Scene *))
{
...
*/
	...
	if(phase==0)
	{
		...
		if(needsRefresh)
		{
			...
			improveBig=MAX(0.3,improveBig-0.005);
			improveInaccurate=MIN(2,improveInaccurate+0.005);
		...
		else
		{
			...
			improveBig=MIN(1,improveBig+0.05);
			improveInaccurate=MAX(0.99,improveInaccurate-0.05);
		}
	}
	assert(__staticReflectors->check());
	if(needsRefresh)
	{
		refreshFormFactorsFromUntil(source,wantedAccuracy,endfunc); // pokud neni phase==0, prostredni parametr muze byt libovolny
	}
	assert(__staticReflectors->check());
	if(phase==0)
	{
...
bool Scene::distribute(real maxError)
{
	...
	while(1)
	{
		Node *source=staticReflectors.best(true,0.000001,0,0);
...
bool Scene::improveStatic(bool endfunc(Scene *))
{
	DBGLINE
	bool improved=false;
...
bool Scene::finishStaticImprovement()
{
	...
	{
		assert(phase>0);
		real e=energyFromDistributedUntil(improvingStatic,falsefunc);
		assert(e); e=e;
		assert(phase==0);
		improvingStatic=NULL;
		return true;
