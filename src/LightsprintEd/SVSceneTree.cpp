...
void SVSceneTree::updateContent(rr_gl::RRSolverGL* solver)
{
	...
	if (solver)
	{
		SetItemText(lights,wxString::Format(_("%zd lights"),solver?solver->getLights().size():0));
	...
	if (solver)
	{
		SetItemText(staticObjects,wxString::Format(_("%zd static objects"),solver?solver->getStaticObjects().size():0));
	...
	if (solver)
	{
		SetItemText(dynamicObjects,wxString::Format(_("%zd dynamic objects"),solver?solver->getDynamicObjects().size():0));
...
{
	ItemData* data = item.IsOk() ? (ItemData*)GetItemData(item) : nullptr;
		...
		case ST_OBJECT:
			{
				...
				if (object->isDynamic)
				{
					...
					if (preTransform)
					{
						...
						object->getCollider()->getMesh()->getAABB(nullptr,nullptr,&localCenter);
...
void SVSceneTree::OnContextMenuCreate(wxTreeEvent& event)
{
	...
	if (entityIdsUniform)
	{
			...
			|| (temporaryContext.IsOk() && GetItemParent(temporaryContext)==dynamicObjects))
		{
			...
			if (selectedExactlyAllStaticObjects || svframe->userPreferences.testingBeta) // is safe only for all objects at once because lightmap.texcoord is in material, not in RRObject, we can't change only selected objects without duplicating materials
...
rr::RRObject* SVSceneTree::addMesh(rr::RRMesh* mesh, wxString name, bool inFrontOfCamera, bool twoSided)
{
	...
	bool aborting = false;
	rr::RRCollider* collider = rr::RRCollider::create(mesh,nullptr,rr::RRCollider::IT_LINEAR,aborting);
...
void SVSceneTree::runContextMenuAction(unsigned actionCode, const EntityIds contextEntityIds)
{
	...
	switch (actionCode)
	{
		...
		case CM_OBJECTS_UNWRAP:
			if (solver)
			{
				...
				if (svframe->unwrapDlg.ShowModal()==wxID_OK)
				{
					...
					svframe->m_canvas->addOrRemoveScene(nullptr,true,containsStaticObject(selectedObjectsAndInstances)); // calls svframe->updateAllPanels();
				}
			}
			break;
			...
			if (solver)
			{
				...
				if (svframe->bakeDlg.ShowModal()==wxID_OK)
				{
					...
					for (unsigned i=0;i<selectedObjects.size();i++)
						if (selectedObjects[i]->getCollider()->getMesh()->getNumVertices())
						{
							...
							selectedObjects[i]->illumination.getLayer(tmpLayer) = w*h
								? rr::RRBuffer::create(rr::BT_2D_TEXTURE,w,h,1,ldm?rr::BF_RGB:rr::BF_RGBF,true,nullptr) // A is only for debugging, F prevents clamping to 1 in very bright regions
								: rr::RRBuffer::create(rr::BT_VERTEX_BUFFER,selectedObjects[i]->getCollider()->getMesh()->getNumVertices(),1,1,rr::BF_RGBF,false,nullptr);
						}
					...
					else // LDM
					{
						...
						solver->updateLightmaps(tmpLayer,-1,-1,&params,&svs.lightmapFilteringParameters);
						solver->setEnvironment(oldEnv,nullptr,svs.skyboxRotationRad);
						delete newEnv;
						...
						for (unsigned i=0;i<selectedObjects.size();i++)
							if (selectedObjects[i]->getCollider()->getMesh()->getNumVertices())
							{
								...
								selectedObjects[i]->illumination.getLayer(tmpLayer) = nullptr;
							}
		...
		case CM_OBJECTS_MERGE_BY_MATERIALS:
			{
				...
				solver->setStaticObjects(newObjects,nullptr);
				solver->setDynamicObjects(newObjects);

				svframe->m_canvas->addOrRemoveScene(nullptr,false,containsStaticObject(selectedObjects)); // calls svframe->updateAllPanels();
			}
			break;
		...
		case CM_OBJECTS_ADD_PLANE:
			{
				addMesh(rr::RRMeshArrays::plane(),_("plane"));
			}
			break;
		case CM_OBJECTS_ADD_RECTANGLE:
			{
				addMesh(rr::RRMeshArrays::rectangle(),_("rectangle"));
			}
			break;
		case CM_OBJECTS_ADD_BOX:
			{
				addMesh(rr::RRMeshArrays::box(),_("box"));
			}
			break;
		case CM_OBJECTS_ADD_SPHERE:
			{
				addMesh(rr::RRMeshArrays::sphere(),_("sphere"));
			}
			break;
		...
		case CM_OBJECTS_DELETE_DIALOG:
			...
			if (svframe->deleteDlg.ShowModal()==wxID_OK)
			{
				...
				svframe->m_canvas->addOrRemoveScene(nullptr,true,containsStaticObject(selectedObjectsAndInstances)); // calls svframe->updateAllPanels(); // necessary at least after deleting empty facegroups
			}
			break;
			...
			if (solver)
			{
				...
				// delete objects
				{
					...
					if (newList.size()!=allObjects.size())
					{
						...
						solver->setStaticObjects(newList,nullptr);
	...
	svframe->OnAnyChange(SVFrame::ES_CONTEXT_MENU,nullptr,nullptr,actionCode);
}
