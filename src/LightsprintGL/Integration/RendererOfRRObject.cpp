...
RendererOfRRObject::RendererOfRRObject(const rr::RRObject* objectImporter, rr::RRDynamicSolver* radiositySolver, const rr::RRScaler* scaler, bool useBuffers)
{
	...
	params.light = NULL;
	params.renderingShadowCasters = false;
...
void RendererOfRRObject::setLightingShadowingFlags(const rr::RRLight* light, bool renderingShadowCasters)
{
	params.light = light;
	params.renderingShadowCasters = renderingShadowCasters;
}
...
/*
//!!! incompatible with useOriginalScene()
	if(testLightingShadowingFlags)
	{
		dontUseIndexed = true;
	...
	if(!dontUseNonBuffered)
	{
		...
		for(unsigned triangleIdx=params.firstCapturedTriangle;triangleIdx<params.lastCapturedTrianglePlus1;triangleIdx++)
		{
			...
			// vypnuto, doufaje ze nas nikdo nevola (krome sceneVieweru), tudiz to nezpomali prilis
			{
				...
				{
					material = params.object->getTriangleMaterial(triangleIdx,params.light,NULL);
				}
				else
				{
					...
					{
						if(material = params.object->getTriangleMaterial(triangleIdx,params.light,params.scene->getObject(i)))
							break;
					}
				}
