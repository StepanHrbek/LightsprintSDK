										...
										)))
								{
									...
									else
									{
										...
										else
										{
											...
											//unsigned mirrorW = (viewport[2]+1)/2;
											//unsigned mirrorH = (viewport[3]+1)/2;
											// b) pot mirror (approx half resolution) for better autogenerated mipmaps, stable, saves one lookup in shader
											int mirrorW = 1; while (mirrorW*3<=viewport[2]*2) mirrorW *= 2;
											int mirrorH = 1; while (mirrorH*3<=viewport[3]*2) mirrorH *= 2;

											objectBuffers.mirrorColorMap = rr::RRBuffer::create(rr::BT_2D_TEXTURE,mirrorW,mirrorH,1,rr::BF_RGBA,true,RR_GHOST_BUFFER);
