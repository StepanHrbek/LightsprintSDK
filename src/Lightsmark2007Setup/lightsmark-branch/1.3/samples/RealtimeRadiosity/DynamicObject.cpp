...
#ifdef OPTIMIZED_RDO
		// optimized renderer, may increase memory usage but reduce draw calls
		d->singleObjects = adaptObjectsFrom3DS(d->model);
		rr::RRObject** objects = new rr::RRObject*[d->singleObjects->size()];
		for(unsigned i=0;i<d->singleObjects->size();i++) objects[i] = (*d->singleObjects)[i].object;
		d->multiObject = rr::RRObject::createMultiObject(objects,d->singleObjects->size(),rr::RRCollider::IT_LINEAR,0,true,NULL);
		delete[] objects;
		d->rendererWithoutCache = new rr_gl::RendererOfRRObject(d->multiObject,NULL,NULL,true);
...
#define IS_DIGIT(c) ((c)>='0' && (c)<='9')
			...
			// workaround for Catalyst bug, observed on HD2xxx, HD3xxx
...
#ifdef OPTIMIZED_RDO
	// optimized render
	rr_gl::RendererOfRRObject::RenderedChannels channels;
	channels.NORMALS = uberProgramSetup.LIGHT_DIRECT;
	rendererWithoutCache->setRenderedChannels(channels);
